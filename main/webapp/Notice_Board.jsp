		    ...
		    <table>
		    	...
		    	List<CommentVO> comList = dao.getComments(list.get(i).getPNum());%>
		    		<tr class="postList">
		    			...
		    			<td id="title"><a href="read.jsp?pNum=<%=list.get(i).getPNum()%>&pageNum=<%=pageNum%>"><%=list.get(i).getTitle() %>[<%=comList.size() %>]</a></td>
			...
			<%
	    		if(cnt != 0) {
		    		...
		    		if(startPage > pageBlock) { %>
		    			<a href="Notice_Board.jsp?pageNum=<%=startPage - pageBlock%>">이전</a>
		    		<%}
	    			// 몇번 페이지로 갈 것인지 번호를 a태그로 생성
	    			for(int i = startPage; i <= endPage; i++) { %>
	    				<a href="Notice_Board.jsp?pageNum=<%=i%>"><%=i %></a>
	    			<%}
	    			
	    			// 10페이지 건너뛰는 버튼
	    			// 남은 페이지가 10 이하라면 다음으로 가는 버튼을 만들 필요가 없다.
	    			if(endPage < pageCnt) { %>
	    				<a href="Notice_Board.jsp?pageNum=<%=startPage + pageBlock%>">다음</a>
	    			<%}
	    		}
  			%>
	    </div>
